...
describe FiscalYear do
  ...
  describe "event creation" do
    before(:each) do
      @event = mock_model(Event, :event_lines => [],
                                 :save => true)
      @event_hsh = {:event_date => @fiscal_year.start_date,
       :description => "Tilinavaus (foo)"}

      @events = []
      @fiscal_year.stub!(:events).and_return(@events)
  ...
  describe "ordered_events" do
    before(:each) do
      @events = [:foo]
      @fiscal_year.should_receive(:events).and_return(@events)
      @events.should_receive(:order).with("event_date, receipt_number").and_return(@events)
      @events.should_receive(:includes).with({:event_lines => :account}).and_return(@events)
    end

    describe "without params" do
      it "should return ordered events" do
        @fiscal_year.ordered_events.should == @events
      end
    end

    describe "with start date" do
      it "should return events later than start date" do
        @events.should_receive(:where).with(["event_date >= ?", Date.new(2012,1,1)]).and_return(@events)
        @fiscal_year.ordered_events("20120101", "").should == @events
      end
    end

    describe "with start and end date" do
      it "should return events later than start date and earlier than end date" do
        @events.should_receive(:where).with(["event_date >= ?", Date.new(2012,1,1)]).and_return(@events)
        @events.should_receive(:where).with(["event_date <= ?", Date.new(2012,7,1)]).and_return(@events)
        @fiscal_year.ordered_events("20120101", "20120701").should == @events
      end
    end
  end
end
